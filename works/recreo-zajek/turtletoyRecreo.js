...
const drawFrame = Math.random() > 0.5;
...
mask = mask.map((s, i) => {
  ...
  while (true) {
    ...
    if (idx >= 0) {
      ...
      var max = buffer_size;
      var t = d / max;
      r = r.replaceAt(idx, t >= Math.random() * 0.75 ? " " : "y");
    } else {
      break;
    }
  }
...
function tile(type, x, y) {
  if (!(type & 0b10000)) {
    const t1 = type >> 2,
      t2 = type & 0b11;
    if (t1 == 0b10) {
      // try to reuse as much turtles as possible
...
function buffer_step(buffer) {
  const nb = [],
    rb = (buffer, x, y) => (x >= 0 && y >= 0 ? buffer[x][y] : 0);
  for (let x = 0; x < buffer_size; x++) {
    nb[x] = [];
    for (let y = 0; y < buffer_size; y++) {
      nb[x][y] = buffer[x][y];
...
function draw_line(xs, ys, xe, ye, x, y) {
  (xs = (x + xs - 0.5) * cell_size), (ys = (y + ys - 0.5) * cell_size);
  (xe = (x + xe - 0.5) * cell_size), (ye = (y + ye - 0.5) * cell_size);
  const ks = xs.toFixed(4) + ys.toFixed(4),
    ke = xe.toFixed(4) + ye.toFixed(4);
  ...
  if (ks != ke && !hashMap[ks + ke] && !hashMap[ke + ks]) {
    ...
    if (turtleMap[ks]) {
      ...
      turtleMap[ke] = turtleMap[ks];
      turtleMap[ks] = false;
    } else if (turtleMap[ke]) {
      turtleMap[ke].goto(xs, ys);
      turtleMap[ks] = turtleMap[ke];
      turtleMap[ke] = false;
    } else {
...
function walk(i) {
  ...
  const x = i % buffer_size,
    y = (i / buffer_size) | 0;
  const xc = x - buffer_size / 2 + 0.5,
    yc = buffer_size / 2 - y - 0.5;

  tile(buffer[x][y], xc, yc);

  if (buffer[x][y] & 0b10000) {
    // border for white squares
    draw_border(
      x > 0 && !(buffer[x - 1][y] & 0b10000),
      y > 0 && !(buffer[x][y - 1] & 0b10000),
      x < buffer_size - 2 && !(buffer[x + 1][y] & 0b10000),
      y < buffer_size - 2 && !(buffer[x][y + 1] & 0b10000),
      xc,
      yc
    );
  }
