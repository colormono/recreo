...
int  nbrParticles = 5000; // 20000; // 6000;
PImage img;
...
void setup() {
  img = loadImage("el-beso.jpg");
  //img = loadImage("color.jpg");

  size(600, 800); // this has to match the size of the image...
...
void doPhysics() {
  ...
  for (int i = 0; i < nbrParticles; ++i) {
    ...
    if (px >= 0 && px < img.width && py >= 0 && py < img.height) {
      int v = (int) brightness(img.pixels[ py*img.width + px ]);
...
void draw() {
  ...
  for (int i = 0; i < nbrParticles; ++i) {
    ...
    float s = map(pb, 0, 255, -1, 2);

    //stroke(ph+90, ps, pb);
    stroke(0, 90, 90);
    //ellipse(particles[i].x+e, particles[i].y+e, medRadius+s, medRadius+s);
    ...
    //stroke(ph, ps, pb);
    ellipse(particles[i].x, particles[i].y, medRadius+s, medRadius+s);
